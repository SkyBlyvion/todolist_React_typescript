...
const Login:React.FC = () => {
  ...
  const handleSubmit = (event: SyntheticEvent) => {
    ...
    setIsLoading(true);
    axios.post(`${API_ROOT}/login`, { email, password }).then((response)=>{
  ...
  return (
    <div className='flex flex-1 flex-col h-screen justify-start items-center bg-brown_dark'>
      ...
      <form onSubmit={handleSubmit} className='w-[80%] md:w-1/2 lg:w-4/12'>
        ...
        <Link to='/register' className='text-white hover:text-yellow_hover '>Pas de compte ? Enregistrez-vous</Link>
